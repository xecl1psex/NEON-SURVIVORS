...
const MARKET=[
 ...
 {id:'magnet',icon:'magnet',name:'МАГНИТНЫЙ ЗАХВАТ',desc:'+10% радиус сбора за уровень',max:5,base:70,growth:1.7,apply:(p,l)=>{/* Магнит теперь рассчитывается в recalcMagnetRange */}},
...
function applyMarket(p){for(const it of MARKET){const l=marketLevels[it.id]||0;if(l>0&&it.apply)it.apply(p,l);}recalcMagnetRange();}
...
let VW=960,VH=600;
// Бесконечный мир - камера следует за игроком
let camera={x:0,y:0};
function resizeCanvas(){const dpr=Math.min(window.devicePixelRatio||1,2);VW=window.innerWidth;VH=window.innerHeight;cvs.style.width=VW+'px';cvs.style.height=VH+'px';cvs.width=Math.floor(VW*dpr);cvs.height=Math.floor(VH*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);}
...
const PERKS=[
 ...
 {id:'vacuum',icon:'magnet',name:'ВАКУУМ',desc:'Радиус сбора опыта x2',apply:p=>{/* Магнит теперь рассчитывается в recalcMagnetRange */}},
...
function openPerkMenu(){
  ...
  if(!remaining.length){perkQueue=0;player.hp=Math.min(player.maxHp+30,player.hp+30);state='playing';
    document.body.classList.add('state-playing');
    document.body.classList.remove('state-paused');
    document.getElementById('abilities').classList.add('show');return;}
  state='perks';
  ...
  opts.forEach(pk=>{const d=document.createElement('div');d.className='opt new';
    ...
    d.onclick=()=>{pk.apply(player);player.perksTaken.push(pk.id);if(pk.id==='vacuum')player.perks.vacuum=true;perkQueue--;updateAchProgress('perks',player.perksTaken.length);hideAllOverlays();sfx('evo');updatePerkHud();recalcMagnetRange();state='playing';
      document.body.classList.add('state-playing');
      document.body.classList.remove('state-paused');
      document.getElementById('abilities').classList.add('show');};
    c.appendChild(d);});
...
function makePlayer(charKey){const ch=CHARACTERS[charKey];
 ...
 // Применяем базовые статы персонажа
 ch.apply(p);
 // После применения бонусов персонажа устанавливаем HP на максимум
 p.hp = p.maxHp;
 // Применяем достижения (некоторые могут менять стартовый уровень и HP)
 for(const id in achUnlocked){const a=ACHIEVEMENTS.find(x=>x.id===id);if(a&&a.rewardApply)a.rewardApply(p);}
 // Если уровень был изменён достижением, корректируем xpNeeded
 if(p.level > 1) {
   let needed = 8;
   for(let i = 1; i < p.level; i++) {
     needed = Math.floor(needed * 1.35) + 2;
   }
   p.xpNeeded = needed;
   p.xp = 0;
 }
 // Синхронизируем HP с maxHp после всех модификаторов
 p.hp = p.maxHp;
 applyMarket(p);grantStartContainers(p);recalcMagnetRange();
...
function startTerminal(){const m=120;let x=player.x+(Math.random()-.5)*VW*0.6,y=player.y+(Math.random()-.5)*VH*0.6,tries=0;do{x=player.x+(Math.random()-.5)*VW*0.8;y=player.y+(Math.random()-.5)*VH*0.8;tries++;}while(Math.hypot(x-player.x,y-player.y)<220&&tries<25);terminal={x,y,progress:0,life:TERMINAL_LIFE};showBonusToast('terminal','ТЕРМИНАЛ ДАННЫХ','Встань рядом и удерживай позицию','#05d9e8');sfx('hack');}
...
function corpDrop(){const n=4+Math.floor(wave/3),sm=BIOMES[currentBiome].speedMult;for(let i=0;i<n;i++){const side=Math.floor(Math.random()*4);let x,y;if(side===0){x=player.x-Math.random()*VW*0.5+Math.random()*VW*0.5;y=player.y-VH*0.5-30;}else if(side===1){x=player.x+VW*0.5+30;y=player.y-Math.random()*VH*0.5+Math.random()*VH*0.5;}else if(side===2){x=player.x-Math.random()*VW*0.5+Math.random()*VW*0.5;y=player.y+VH*0.5+30;}else{x=player.x-VW*0.5-30;y=player.y-Math.random()*VH*0.5+Math.random()*VH*0.5;}const s=1+wave*.12;enemies.push({x,y,type:'elite',r:16,hp:40*s,maxHp:40*s,speed:(50+wave*3)*sm,dmg:12+wave*2,xp:10+Math.floor(wave/2),color:'#ffd319',elite:true,hitFlash:0,frozen:0,poisoned:0,slowTimer:0,burn:0,orbitCd:0});}showBonusToast('bell','ДЕСАНТ КОРПОРАЦИИ','Элитный отряд на подходе','#ff2a6d');sfx('alarm');screenShake=15;}
...
function pauseGame(){
  ...
  // Добавляем класс на body для показа кнопок через CSS
...
function resumeGame(){
  ...
  state='playing';
  hideAllOverlays();
  // Убираем класс паузы, добавляем класс игры
  document.body.classList.remove('state-paused');
  document.body.classList.add('state-playing');
  if(actx)Music.start();
}
...
function startGame(charKey){try{if(!unlockedChars.has(charKey))return;initAudio();sfx('select');player=makePlayer(charKey);enemies=[];bullets=[];xpOrbs=[];particles=[];eBullets=[];bosses=[];wave=1;waveTimer=0;waveDuration=25;gameTime=0;kills=0;bossKills=0;hackCount=0;screenShake=0;damageFlash=0;dmgBonusTimer=0;comboCount=0;comboMultiplier=1;comboTimer=0;comboMax=1;eventTimer=40;activeEvent=null;terminal=null;runBonusCredits=0;perkQueue=0;joy.active=false;currentBiome=0;setBiomeHUD(0);abilityCooldowns={};prevLevelForUnlock=1;unlockToastTimer=0;evoToastTimer=0;achToastTimer=0;bonusToastTimer=0;evolvedSet=new Set();runAchievementsChecked=new Set();totalStats.games++;isVictory=false;
 ...
 const ch=CHARACTERS[charKey];document.getElementById('charname').innerHTML=icon(ch.icon,14,ch.color)+`<span style="color:${ch.color};text-shadow:0 0 6px ${ch.color}">${ch.name}</span>`;updateWeaponsUI();buildAbilitiesUI();updatePerkHud();startTelemetry();state='playing';
  // Устанавливаем класс состояния игры
  document.body.classList.add('state-playing');
  document.body.classList.remove('state-paused');
  Music.start();}catch(e){crash(e);}}
...
function spawnEnemy(){if(enemies.length>=MAX_ENEMIES)return;const b=BIOMES[currentBiome];const side=Math.floor(Math.random()*4);let x,y;if(side===0){x=player.x-Math.random()*VW*0.5+Math.random()*VW*0.5;y=player.y-VH*0.5-30;}else if(side===1){x=player.x+VW*0.5+30;y=player.y-Math.random()*VH*0.5+Math.random()*VH*0.5;}else if(side===2){x=player.x-Math.random()*VW*0.5+Math.random()*VW*0.5;y=player.y+VH*0.5+30;}else{x=player.x-VW*0.5-30;y=player.y-Math.random()*VH*0.5+Math.random()*VH*0.5;}const s=1+wave*.12;const type=pickEnemyType(wave,b.mix);
...
function generateUpgrades(){const pool=[];for(const k in WEAPONS){const w=WEAPONS[k];if(w.evo)continue;const cur=player.weapons[k]||0;if(cur<w.maxLvl){const owned=cur>0,nl=cur+1;pool.push({new:!owned,name:w.name+(owned?' ур.'+nl:''),icon:w.icon,desc:w.desc+(owned?' (улучшение)':' (новое!)'),tag:owned?'АПГРЕЙД':'НОВОЕ',apply:()=>{player.weapons[k]=nl;}});}}for(const k in PASSIVES){const cur=player.passives[k]||0;if(cur<PASSIVES[k].max)pool.push({new:cur===0,name:PASSIVES[k].name+(cur>0?' ур.'+(cur+1):''),icon:PASSIVES[k].icon,desc:PASSIVES[k].desc,tag:cur>0?'АПГРЕЙД':'НОВОЕ',apply:()=>{player.passives[k]=(player.passives[k]||0)+1;applyPassive(k);recalcMagnetRange();}});}return pool.sort(()=>Math.random()-.5).slice(0,3);}
function applyPassive(k){const l=player.passives[k];if(k==='might')player.dmgMult=1+l*.15;if(k==='speed')player.speedMult=1+l*.10;if(k==='maxhp'){player.maxHp+=20*l;player.hp=player.maxHp;}if(k==='firerate')player.fireRateMult=Math.pow(.9,l);if(k==='magnet'){/* Магнит теперь рассчитывается в recalcMagnetRange */}if(k==='regen')player.regen=(player.charKey==='monk'?.5:0)+l*.5;if(k==='crit')player.critChance=l*.05;if(k==='armor')player.armor=l;}
function recalcMagnetRange(){if(!player||!player.passives)return;let base=80,mult=1;const ml=player.passives.magnet||0;if(ml>0)mult*=1+ml*.5;const marketMagnetLvl=marketLevels['magnet']||0;if(marketMagnetLvl>0)mult*=1+0.10*marketMagnetLvl;if(player.perks&&player.perks.vacuum)mult*=2;player.magnetRange=base*mult;}
function showLevelUp(){state='levelup';const opts=generateUpgrades(),c=document.getElementById('options');c.innerHTML='';if(!opts.length){const d=document.createElement('div');d.className='opt';d.innerHTML=`<div class="ic">${icon('heart',36)}</div><div class="nm">+50 HP</div><div class="ds">Всё прокачано</div><div class="tag">БОНУС</div>`;d.onclick=()=>{player.hp=Math.min(player.maxHp+50,player.hp+50);player.maxHp+=50;hideAllOverlays();sfx('lvl');state='playing';
      document.body.classList.add('state-playing');
      document.body.classList.remove('state-paused');
      document.getElementById('abilities').classList.add('show');updateAbilitiesUI();};c.appendChild(d);}else opts.forEach(o=>{const d=document.createElement('div');d.className='opt'+(o.new?' new':'');d.innerHTML=`<div class="ic">${icon(o.icon,36)}</div><div class="nm">${o.name}</div><div class="ds">${o.desc}</div><div class="tag">${o.tag}</div>`;d.onclick=()=>{o.apply();hideAllOverlays();sfx('lvl');state='playing';
        document.body.classList.add('state-playing');
        document.body.classList.remove('state-paused');
        document.getElementById('abilities').classList.add('show');updateWeaponsUI();updateAbilitiesUI();checkEvolutions();};c.appendChild(d);});hideAllOverlays();document.getElementById('levelup').classList.add('show');}
...
function update(dt){if(!player)return;if(perkQueue>0){openPerkMenu();return;}gameTime+=dt;player.invuln=Math.max(0,player.invuln-dt);player.shieldTime=Math.max(0,player.shieldTime-dt);damageFlash=Math.max(0,damageFlash-dt);screenShake=Math.max(0,screenShake-dt*20);if(player.regen>0)player.hp=Math.min(player.maxHp,player.hp+player.regen*dt);if(!isFinite(player.x)||!isFinite(player.y)){player.x=camera.x+VW/2;player.y=camera.y+VH/2;}if(!isFinite(player.xpNeeded)||player.xpNeeded<=0)player.xpNeeded=10;
 ...
 bullets=bullets.filter(bl=>bl.life>0&&bl.x>camera.x-100&&bl.x<camera.x+VW+100&&bl.y>camera.y-100&&bl.y<camera.y+VH+100);
 ...
 eBullets=eBullets.filter(eb=>eb.life>0&&eb.x>camera.x-50&&eb.x<camera.x+VW+50&&eb.y>camera.y-50&&eb.y<camera.y+VH+50);
 ...
 xpOrbs=xpOrbs.filter(o=>!o.dead&&(o.x>camera.x-200||o.x<camera.x+VW+200||o.y>camera.y-200||o.y<camera.y+VH+200));
...
// Амбиентные частицы привязаны к камере — они "живут" в экранных координатах.
// Раньше были привязаны к мировому (0,0) и уезжали вместе со спавном игрока.
function drawAmbient(t,b){const W=VW,H=VH;const cx=camera.x,cy=camera.y;
 if(b.ambient==='dust'){for(let i=0;i<26;i++){const sx=cx+(i*173.3)%W,sy=cy+((i*97.7)+t*(14+(i%4)*8))%H;ctx.fillStyle=i%3===0?'rgba(255,42,109,.18)':'rgba(5,217,232,.16)';ctx.fillRect(sx,sy,2,2);}}
 else if(b.ambient==='embers'){for(let i=0;i<30;i++){const sx=cx+(i*167.7)%W,sy=cy+H-(((i*89.3)+t*(30+(i%5)*14))%(H+40));const fl=.4+.6*Math.abs(Math.sin(t*6+i*2));ctx.fillStyle=`rgba(255,158,0,${.25*fl})`;ctx.fillRect(sx+Math.sin(t*2+i)*8,sy,2,2);}}
 else if(b.ambient==='matrix'){for(let i=0;i<24;i++){const sx=cx+(i*40.3)%W,sy=cy+((i*131.7)+t*(80+(i%4)*40))%(H+60)-30;ctx.fillStyle=`rgba(118,255,3,${.12+.1*Math.abs(Math.sin(t*3+i))})`;ctx.fillRect(sx,sy,2,10+(i%3)*6);}}
 else if(b.ambient==='glitch'){for(let i=0;i<18;i++){const sx=cx+(i*211.3)%W,sy=cy+(i*167.9)%H;ctx.fillStyle=`rgba(185,103,255,${.15+.15*Math.abs(Math.sin(t*5+i*3))})`;ctx.fillRect(sx,sy,2,2);}const g=Math.sin(t*13.7)+Math.sin(t*7.3);if(g>1.85){for(let i=0;i<3;i++){const y=cy+Math.random()*H;ctx.fillStyle='rgba(185,103,255,.06)';ctx.fillRect(cx,y,W,2+Math.random()*6);}}}}
function drawGameBG(t,b){const W=VW,H=VH;const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,b.bg[0]);g.addColorStop(.5,b.bg[1]);g.addColorStop(1,b.bg[2]);ctx.fillStyle=g;ctx.fillRect(camera.x,camera.y,W,H);const off=(t*26)%48;const startX=Math.floor(camera.x/48)*48-off;const startY=Math.floor(camera.y/48)*48-off;ctx.lineWidth=1;for(let x=startX;x<camera.x+W+48;x+=48){const br=((x/48)|0)%4===0;ctx.strokeStyle=br?b.gridB:b.gridA;ctx.beginPath();ctx.moveTo(x,camera.y);ctx.lineTo(x,camera.y+H);ctx.stroke();}for(let y=startY;y<camera.y+H+48;y+=48){const br=((y/48)|0)%4===0;ctx.strokeStyle=br?b.gridB:b.gridA;ctx.beginPath();ctx.moveTo(camera.x,y);ctx.lineTo(camera.x+W,y);ctx.stroke();}drawAmbient(t,b);
 // Шторм — теперь тоже привязан к камере (экранные координаты + camera.x/y)
 if(activeEvent&&activeEvent.type==='storm'){ctx.strokeStyle='rgba(118,255,3,.28)';ctx.lineWidth=1;for(let i=0;i<16;i++){const sx=camera.x+((i*97.3+t*320)%(W+200))-100,sy=camera.y+(i*61.7)%H;ctx.beginPath();ctx.moveTo(sx,sy);ctx.lineTo(sx-44,sy+14);ctx.stroke();}}}
...
function render(t){
  ctx.save();
  const shk=settings.shake?screenShake:0;
  if(shk>0)ctx.translate((Math.random()-.5)*shk,(Math.random()-.5)*shk);
  if(state==='menu'||state==='charselect'||state==='achievements'||state==='settings'||state==='market'){drawMenuBG(t);ctx.restore();return;}
  camera.x=player.x-VW/2;camera.y=player.y-VH/2;
  ctx.save();
  ctx.translate(-camera.x,-camera.y);
  // ===== ВСЁ, ЧТО РИСУЕТСЯ В МИРОВЫХ КООРДИНАТАХ =====
  drawGameBG(t,BIOMES[currentBiome]);
  drawTerminal(t);
  ...
  // ===== КОНЕЦ МИРОВЫХ КООРДИНАТ =====
  ctx.restore();

  // ===== ЭКРАННЫЕ ЭФФЕКТЫ (НЕ зависят от камеры) =====
  // Эффект blackout — центрируется на игроке (он всегда в центре экрана)
  if(activeEvent&&activeEvent.type==='blackout'&&player){
    const g=ctx.createRadialGradient(VW/2,VH/2,90,VW/2,VH/2,Math.max(VW,VH)*.55);
    g.addColorStop(0,'rgba(1,1,43,0)');
    g.addColorStop(1,'rgba(1,1,43,.9)');
    ctx.fillStyle=g;
    ctx.fillRect(0,0,VW,VH);
  }
  // Виртуальный джойстик — экранные координаты
  drawJoystick();
  // Вспышка урона — на весь экран
  if(damageFlash>0){
    ctx.fillStyle=`rgba(255,42,109,${damageFlash*.3})`;
    ctx.fillRect(0,0,VW,VH);
  }
  ctx.restore();
}

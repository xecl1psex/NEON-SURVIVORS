...
function showLevelUp(){state='levelup';const opts=generateUpgrades(),c=document.getElementById('options');c.innerHTML='';if(!opts.length){const d=document.createElement('div');d.className='opt';d.innerHTML=`<div class="ic">${icon('heart',36)}</div><div class="nm">+50 HP</div><div class="ds">Всё прокачано</div><div class="tag">БОНУС</div>`;d.onclick=()=>{player.hp=Math.min(player.maxHp+50,player.hp+50);player.maxHp+=50;hideAllOverlays();sfx('lvl');state='playing';updateAbilitiesUI();};c.appendChild(d);}else opts.forEach(o=>{const d=document.createElement('div');d.className='opt'+(o.new?' new':'');d.innerHTML=`<div class="ic">${icon(o.icon,36)}</div><div class="nm">${o.name}</div><div class="ds">${o.desc}</div><div class="tag">${o.tag}</div>`;d.onclick=()=>{o.apply();hideAllOverlays();sfx('lvl');state='playing';updateWeaponsUI();updateAbilitiesUI();checkEvolutions();};c.appendChild(d);});hideAllOverlays();document.getElementById('levelup').classList.add('show');}

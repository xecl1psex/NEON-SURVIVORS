...
function render(t){ctx.save();const shk=settings.shake?screenShake:0;if(shk>0)ctx.translate((Math.random()-.5)*shk,(Math.random()-.5)*shk);if(state==='menu'||state==='charselect'||state==='achievements'||state==='settings'||state==='market'){drawMenuBG(t);ctx.restore();return;}camera.x=player.x-VW/2;camera.y=player.y-VH/2;ctx.save();ctx.translate(-camera.x,-camera.y);drawGameBG(t,BIOMES[currentBiome]);drawTerminal(t);
